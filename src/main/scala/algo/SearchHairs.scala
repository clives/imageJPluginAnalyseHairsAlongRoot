...
object SearchHairs {
  ...
  def followHair( pixelofinterestovery: Map[ Int, List[ Int]])={
    ...
    def getallhairs( deltatostartwith: List[Delta], filtredpixelofinterestovery: Map[ Int, List[ Int]], intersult: List[((Delta,IDHAIR),List[Int])]=List.empty  ): List[((Delta,IDHAIR),List[Int])] ={
      ...
      if( deltatostartwith.head < 5  || filtredpixelofinterestovery.isEmpty) return intersult
      
      if( filtredpixelofinterestovery(delta).isEmpty )return  getallhairs( deltatostartwith.tail, filtredpixelofinterestovery, intersult)
